...
<html lang="zh-CN">
	...
	<body>
		<header class="ui-page-header">
			<div class="mini-title">充值数据可视化</div>
		</header>
  ...
  <button class="button button-glow button-circle button-action button-jumbo"><i class="fa fa-thumbs-up"></i></button>
		...
		<div class="form-group">
				<label>时间</label>
					<input type="text" data-date-format="yyyy-mm-dd"  name="startTime" class="form-control form-date start-date" value="${startDate}">
					<input type="text" data-date-format="yyyy-mm-dd" name="endTime" class="form-control form-date end-date" value="${endDate}">
				</div>
				<div class="form-group">
	...
	<!-- 	<div class="panel panel-default">
			...
			<div class="panel panel-default">
				...
				<div class="panel-body">
					<table class="table table-bordered table-hover">
						...
						<th class="min-w40">总充值</th>
						<th class="min-w40">今日充值数</th>
						<th class="min-w40">充值人数</th>
						<th class="min-w40">商品1数量</th>
						<th class="min-w40">商品1数量</th>
						<th class="min-w40">商品1数量</th>
						<th class="min-w40">商品1数量</th>
						<th class="min-w40">商品1数量</th>
		...
		<script id="user-data-tmpl" type="text/html">
			{{each userData}}
				<tr>
					<td>{{$value.date}}</td>
	                <td>{{$value.totalRecharge}}</td>
					<td>{{$value.todayRecharge}}</td>
					<td>{{$value.todayRechargePlayerNum}}</td>
					<td>{{$value.scale1}}</td>
					<td>{{$value.scale1}}</td>
					<td>{{$value.scale1}}</td>
					<td>{{$value.scale1}}</td>
				...
				success: function() {
					seajs.use(['lib/jquery', 'lib/highcharts', 'util/bootstrap.datetimepicker.zh-CN', 'util/ajaxPromise', 'util/artTemplate'], function($, highcharts, datetimepicker, ajaxPromise, tmpl) {
						...
						updateData = function() {
							...
							ajaxPromise({
								url: window.basePath + 'data/sxDataRechargeListSearch',
								type: 'GET',
								data: config,
								dataType: 'json'
							}).then(function(data) {
								...
								plotLine(userAddition, {
									...
									series: [{
										name: '总充值(元)',
										data: renderData.totalRecharge
										
									},
									{
										name: '今日充值数(元)',
										data: renderData.todayRecharge
										
									},
									{
										name: '充值人数(人)',
										data: renderData.todayRechargePlayerNum
