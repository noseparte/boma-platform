...
<html lang="zh-CN">
	...
	<body>
		<c:set var="preUrl" value="dataRegisterAnalysisList
									?startTime=${ startTime }
									&endTime=${ endTime }&" />
		<header class="ui-page-header">
			<div class="mini-title">当前位置：注册统计</div>
		</header>
			...
			<table class="table table-hover table-bordered table-condensed">
				<thead>
					<tr class="info">
						...
						<th class="min-w40">type(小时，日，月)</th>
						<th class="min-w40">注册数（创建id）</th>
						<th class="min-w40">活跃数（创建id) </th>
						<th class="min-w40">新增角色数</th>
						<th class="min-w40">新增账户登录游戏数</th>
						<th class="min-w40">新增手机设备数</th>
					</tr>
				</thead>
				<tbody id="topic-list">
					<c:forEach var="registerAnalysis" items="${ registerAnalysisList }">
					<tr data-id="${ registerAnalysis.datetime }">
						...
						<td>${ registerAnalysis.type }</td>
						<td>${ registerAnalysis.registeraccoutcount }</td>
						<td>${ registerAnalysis.activeaccoutcount }</td>
						<td>${ registerAnalysis.registerplayercount }</td>
						<td>${ registerAnalysis.loginaccountcount }</td>
						<td>${ registerAnalysis.maccount }</td>
					</tr>
					</c:forEach>
				</tbody>
			</table>
