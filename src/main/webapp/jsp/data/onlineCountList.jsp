...
<html lang="zh-CN">
	...
	<body>
		<c:set var="preUrl" value="dataOnlineCountList
									?startTime=${ startTime }
									&endTime=${ endTime }&" />
		<header class="ui-page-header">
			<div class="mini-title">当前位置：游戏服务器在线统计</div>
		</header>
			...
			<table class="table table-hover table-bordered table-condensed">
				<thead>
					<tr class="info">
						...
						<th class="min-w40">在线人数（每分钟）</th>
					</tr>
				</thead>
				<tbody id="topic-list">
					<c:forEach var="dataOnlineCount" items="${ dataOnlineCountList }">
					<tr data-id="${ dataOnlineCount.datetime }">
						<td>${ dataOnlineCount.gameid }</td>
						<td>${ dataOnlineCount.gameserverid }</td>
						<td>${ dataOnlineCount.channelid }</td>
						<td>${ dataOnlineCount.platfromid }</td>
						<td>${ dataOnlineCount.onlinecount }</td>
					</tr>
					</c:forEach>
				</tbody>
			</table>
