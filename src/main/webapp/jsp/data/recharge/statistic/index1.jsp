				...
				<div class="col-sm-12">
					<div class="panel panel-default" style="min-height:500px;">
						...
						<table
							...
							<thead>
								<tr style="background:#60686b;">
									...
									<th class="min-w50">总充值</th>
								</tr>
							</thead>
							<tbody>
								<c:forEach var="one" items="${ t1Lst }" varStatus="status">
									<tr>
										<td>${ one.date }</td>
										<td>${ one.number } ${one.unit }</td>
									</tr>
								</c:forEach>
							</tbody>
						</table>
						...
						<table
							...
							style="display:none;" data-graph-container="container"
							data-graph-container-before="1" data-graph-type="line">
							<caption>今日总充值</caption>
							<thead>
								<tr style="background:#60686b;">
									<th class="min-w50">日期</th>
									<th class="min-w80">今日总充值</th>
								</tr>
							</thead>
							<tbody>
								<c:forEach var="one" items="${ t2Lst }" varStatus="status">
									<tr>
										<td>${ one.date }</td>
										<td>${ one.number } ${one.unit }</td>
									</tr>
						...
						<table
							...
							style="display:none;" data-graph-container="container"
							data-graph-container-before="1" data-graph-type="line">
							<caption>今日充值玩家数</caption>
							<thead>
								<tr style="background:#60686b;">
									<th class="min-w50">日期</th>
									<th class="min-w50">今日充值玩家数</th>
								</tr>
							</thead>
							<tbody>
								<c:forEach var="one" items="${ t3Lst }" varStatus="status">
									<tr>
										<td>${ one.date }</td>
										<td>${ one.number } ${one.unit }</td>
									</tr>
