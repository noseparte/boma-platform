...
<html lang="zh-CN">
	...
	<body>
		<c:set var="preUrl" value="dataRetentionAnalysisList
									?startTime=${ startTime }
									&endTime=${ endTime }&" />
		<header class="ui-page-header">
			<div class="mini-title">当前位置：留存统计分析</div>
		</header>
			...
			<table class="table table-hover table-bordered table-condensed">
				<thead>
					<tr class="info">
						...
						<th class="min-w40">留存数（次日）</th>
						<th class="min-w40">留存率（次日）</th>
					</tr>
				</thead>
				<tbody id="topic-list">
					<c:forEach var="dataRetentionAnalysis" items="${ dataRetentionAnalysisList }">
					<tr data-id="${ dataRetentionAnalysis.datetime }">
						<td>${ func:formatDate(dataRetentionAnalysis.datetime) }</td>
						<td>${ dataRetentionAnalysis.gameid }</td>
						<td>${ dataRetentionAnalysis.gameserverid }</td>
						<td>${ dataRetentionAnalysis.channelid }</td>
						<td>${ dataRetentionAnalysis.platfromid }</td>
						<td>${ dataRetentionAnalysis.retentioncount }</td>
						<td>${ dataRetentionAnalysis.retentionrate }</td>
					</tr>
					</c:forEach>
				</tbody>
			</table>
