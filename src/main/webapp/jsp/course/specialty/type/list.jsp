                ...
                <div class="ibox-content">
                    <div id="toolbar" class="btn-group">
                        <div class="pull-left form-inline form-group">
                            <label>专业类型</label>
                            <input type="text" id="type" name="type" class="form-control" placeholder="专业类型" />
...
<script type="text/javascript">
    ...
    //表格请求及数据
    var tableColumns = {
        url: 'api/course/get_specialty_type_list?&tm=' + new Date().getTime(),
        toolbar:'#toolbar',
        method:'post',
        columns: [{
            field: 'id',
    ...
    //跳转到新增页面
    function toAdd(){
        window.location.href = "<%=basePath%>api/course/to_specialty_add_view";
    }

    //操作
    function formatNAMEFun(value, row, index){
        var format_v = "<button type=\"button\" class=\"btn btn-link\" onclick=\"toView('"+row.id+"');\">" + row.eduLevel + "</button>";
        return format_v;
    }
    // 跳转到编辑页面
    function toEdit(){
        ...
        if(id != null && id != ""){
            layer.open({
                ...
                content: ['<%=basePath%>api/course/get_specialty_type_edit?id=' + id]
            })
        }else{
    ...
    function toDel(){
        ...
        layer.confirm('确定删除已选信息吗？', {
            ...
            yes: function(index, layero){
                $.ajax({
                    type: "POST",
                    url: '<%=basePath%>api/course/specialty_type_delete?tm=' +  new Date().getTime(),
                    data: {
                        IDS: idsStr
                    },
                    dataType: 'json',
        ...
        if(id != null && id != ""){
            layer.full(
                layer.open({
                    ...
                    content: ['<%=basePath%>api/course/specialty_award_view?id=' + id],//URL地址
                    //closeBtn: 1,//显示关闭按钮
                    //btn: ['关闭']
                })
            );
