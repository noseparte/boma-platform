...
public final class FilesUtilsDgNew {
	
	private static Logger LOGGER = LoggerFactory.getLogger(FilesUtilsDgNew.class);
	
    /**
     * private constructor method that make class can not be instantiation
     */
    private FilesUtilsDgNew() {
    ...
    public static String FilesUpload_stream(HttpServletRequest request,MultipartFile multipartFile,String filePath) {
    	try {
    		Assert.isTrue(multipartFile != null, "文件不能为空");
     ...
     * @return
     */
    public static String FilesUpdateUpload_stream(HttpServletRequest request,MultipartFile multipartFile,String filePath) {
    	try {
    		Assert.isTrue(multipartFile != null, "文件不能为空");
    		String suffix = multipartFile.getOriginalFilename();
            String absolutePath = getAndSetAbsolutePath(request, filePath, suffix);
    ...
    private static String getAndSetAbsolutePath(HttpServletRequest request, String filePath, String suffix) {
        String savePath = getServerPath(request, filePath) + File.separator + getDataPath() + File.separator;
        checkDir(savePath);
        return savePath + suffix;
    }

    private static String getRelativePath(String filePath, String suffix) {
        return filePath + File.separator + getDataPath() + File.separator + suffix;//example:/files/Upload/20160912/
    }
}