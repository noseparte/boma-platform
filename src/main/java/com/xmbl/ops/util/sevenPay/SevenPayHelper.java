...
public class SevenPayHelper {
	...
	// 日志打印
	private static Logger LOGGER = LoggerFactory.getLogger(SevenPayHelper.class);
	...
	public static String dealSignParam(Map<String, String[]> paramMap) throws Exception {
		...
		for(int i=0; i<keyList.size(); i++)
		{
			...
			if(null != value && value.length > 0 && null != value[0] && !"".equals(value[0]))
			{
...
//				result.append("&").append(key).append("=").append(URLDecoder.decode(value[0],"UTF-8"));
	 ...
	 * 去掉 signMsg 和 signType 两个参数和其他标明的不参与签名的参数，将其他参数按照字母顺序升序排列，再将所有数组值以“&”字符连接起来：
	 * @param paramMap
	 * @return
	 * @throws Exception 
	 */
